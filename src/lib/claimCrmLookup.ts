...
async function fetchCrmRecords(query: string, signal?: AbortSignal): Promise<Record<string, unknown>[] | null> {
  const url = `${CRM_GET_BY_ID}?id=${encodeURIComponent(query)}&pass=${encodeURIComponent(CRM_GET_BY_ID_PASS)}`;
...
/** Per-person coverage from CRM riders (extensions often apply to one insured only). */
function coverageDatesFromRiders(riders: unknown): { startDate: string; endDate: string } | null {
  ...
  for (const item of riders) {
    ...
    const s = normalizeDate(row.startDate);
    const e = normalizeDate(row.endDate);
    const sm = s ? new Date(`${s}T12:00:00`).getTime() : Number.NaN;
    const em = e ? new Date(`${e}T12:00:00`).getTime() : Number.NaN;
    if (Number.isFinite(sm) && Number.isFinite(em) && em < sm) continue;
    if (Number.isFinite(sm) && sm < startMs) {
      startMs = sm;
      start = s;
    }
    if (Number.isFinite(em) && em > endMs) {
      endMs = em;
      end = e;
    }
  }
...
function personCoverageFromRawPolicies(
  policiesRaw: Record<string, unknown>[],
  normalizedId: string,
  rawId: string
): { startDate: string; endDate: string } | null {
  const sorted = [...policiesRaw].sort(
    (a, b) => policyDocNum(b.policyDoc ?? b.PolicyDoc) - policyDocNum(a.policyDoc ?? a.PolicyDoc)
  );
  for (const p of sorted) {
    const customers = Array.isArray(p.customers) ? p.customers : [];
    const person = customers.find((c) =>
      personMatchesId(c as Record<string, unknown>, normalizedId, rawId)
    ) as Record<string, unknown> | undefined;
    if (!person) continue;
    const cov = coverageDatesFromRiders(person.riders);
    if (cov) return cov;
  }
  return null;
}

function applyPersonCoverage(
  policy: ClaimCrmPolicy,
  cov: { startDate: string; endDate: string } | null
): ClaimCrmPolicy {
  if (!cov) return policy;
  return {
    ...policy,
    startDate: cov.startDate || policy.startDate,
    endDate: cov.endDate || policy.endDate,
  };
}

function policyQueryKey(policyId: string): string {
  return policyId.replace(/[^\d]/g, "") || policyId.trim();
}

/**
 * ID lookup omits riders; policy-number lookup includes them.
 * Fetch by policy number so each insured sees their own coverage dates (e.g. one-person extension).
 */
async function enrichPoliciesWithPersonCoverage(
  policies: ClaimCrmPolicy[],
  normalizedId: string,
  rawId: string,
  alreadyRaw?: Record<string, unknown>[]
): Promise<ClaimCrmPolicy[]> {
  const alreadyKey = alreadyRaw?.length
    ? policyQueryKey(pickStr(alreadyRaw[0].fullPolicyID, alreadyRaw[0].policyNumber))
    : "";
  const alreadyCov = alreadyRaw?.length
    ? personCoverageFromRawPolicies(alreadyRaw, normalizedId, rawId)
    : null;
  ...
  try {
    return await Promise.all(
      policies.map(async (policy) => {
        const key = policyQueryKey(policy.fullPolicyID);
        if (alreadyCov && alreadyKey && key === alreadyKey) {
          return applyPersonCoverage(policy, alreadyCov);
        }
        // Original policies without additions usually share one period; still check recent ones.
        if (policy.policyDoc <= 0) {
          const year = Number((policy.endDate || policy.startDate).slice(0, 4));
          if (!Number.isFinite(year) || year < new Date().getFullYear() - 3) return policy;
        }
        try {
          const raw = await fetchCrmRecords(policy.fullPolicyID, controller.signal);
          if (!raw?.length) return policy;
          return applyPersonCoverage(policy, personCoverageFromRawPolicies(raw, normalizedId, rawId));
        } catch {
          return policy;
        }
      })
...
export const claimPolicyFilterCopy = (
  ...
  if (claimType === "trip_shorten") {
    return {
      listHint: "מוצגות נסיעות שהתחילו ב־3 השנים האחרונות — כולל פוליסות שכבר הסתיימו (קיצור נסיעה)",
      emptyTitle: "לא מצאנו נסיעה מתאימה",
      emptyBody:
        "לקיצור נסיעה נדרשת פוליסה שנסיעתה כבר התחילה, גם אם כבר חזרתם (עד 3 שנים אחורה). נסיעות עתידיות בלבד לא מוצגות כאן.",
      upcomingTitle: "נסיעות פעילות כעת",
      pastTitle: "נסיעות שעברו (גם פוליסות שכבר הסתיימו)",
    };
  }
