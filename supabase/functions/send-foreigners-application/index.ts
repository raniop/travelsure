...
serve(async (req) => {
  ...
  try {
    ...
    const workerRows = [
      ...
      row("שם אמצעי", summary.middleName, true),
      row("שם משפחה", summary.lastName, false),
      row("מספר דרכון", summary.passportNo, true),
      row("ארץ הנפקת דרכון", summary.passportCountry, false),
      row("ארץ מוצא", summary.countryOfOrigin, true),
      row("תאריך לידה", summary.birthDate, false),
      row("מין", summary.gender, true),
      row("תאריך ראשון שבוטח", summary.firstInsuranceDate, false),
      row("תאריך כניסה לישראל", summary.entryDate, true),
      row("תקופת ביטוח מ־", summary.insuranceFrom, false),
      row("תקופת ביטוח עד", summary.insuranceTo, true),
      row("עיסוק / תיאור עבודה", summary.workDescription, false),
      row("כתובת", summary.address, true),
      row("טלפון", summary.phone, false),
      row("נייד", summary.mobile, true),
      row("דוא״ל", summary.email, false),
      row("מעוניין בדואר ישראל", summary.preferPostMail, true),
